...
The pipeline imports assets into a temporary keychain, checks profile team/identifier/expiration/push capability/distribution method, archives, exports, verifies with codesign, checks the exported app against the requested team, Bundle ID, version/build and profile certificate/entitlement allowlist, and cleans up signing material on success or failure. Profile App ID prefixes are read independently from the Team ID, including older Apple accounts. These checks do not establish Apple processing, certificate revocation status or real-device installation. Export methods are `app-store-connect` and `release-testing` respectively. Only deliverable IPAs and their verification reports are uploaded; certificates/private keys are not artifacts.
...
The validation job also runs `python3 -m unittest discover -s tests -p "test_ios_*.py" -v` before distribution. Its 12 synthetic-profile tests require no Apple credentials and cannot create signed IPAs. The rules follow [Apple TN3125: provisioning profiles](https://developer.apple.com/documentation/technotes/tn3125-inside-code-signing-provisioning-profiles).
