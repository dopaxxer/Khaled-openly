...
export async function fetchViewer({ signal, force = false } = {}) {
  ...
  inflight = request()
    .then(user => {
      // A logout or forced refresh supersedes both the cache and the answer
      // delivered to callers still waiting on the previous session.
      if (requestGeneration !== generation) return inflight || cachedUser
      cachedUser = user
      hasCached = true
      cachedAt = Date.now()
      return user
