...
function required(name, value) {
  ...
  throw new Error(
    `[openly] ${name} is not set. Set it as a BUILD variable on the Cloudflare ` +
    'Workers project (Settings → Builds → Build variables) to the Supabase project ' +
    'this deployment owns, then redeploy. NEXT_PUBLIC_* values are inlined at build ' +
    'time, so setting them as runtime variables has no effect. ' +
