...
function sweep(now) {
  ...
  // Dropping closed windows is not on its own a bound: a flood of distinct
  // keys inside a single window expires nothing, so the map grew past the cap
  // unchecked -- exactly the case the cap exists for, since the anonymous
  // scopes are keyed by forwarded address. Hold the ceiling by evicting the
  // oldest entries; a Map iterates in insertion order, so those are the
  // buckets closest to resetting anyway. Evicting a live bucket restarts its
  // window, which is the accepted cost of a bounded map in a limiter that is
  // abuse damping rather than a hard quota.
  if (buckets.size < MAX_TRACKED_KEYS) return
  let excess = buckets.size - MAX_TRACKED_KEYS + 1
...
export const RATE_LIMITS = {
  ...
  messagePresence: { limit: 120, windowMs: 60_000 },
