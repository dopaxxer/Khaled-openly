...
async function fetchApple(url, signal) {
  // RequestInit.cache can be rejected by Cloudflare Workers on deployments
  // with an older compatibility date. This module already has its own bounded
  // TTL cache, so do not depend on the runtime-specific fetch cache option.
