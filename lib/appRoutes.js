...
export function classifyAppRoute(slug) {
  ...
  if (segments.length === 2 && segments[0] === 'messages') {
    ...
    return { kind: 'private', key: `messages/${id}`, path: `/messages/${id}`, title: 'محادثة خاصة' }
  }

  if (segments.length === 3 && segments[0] === 'messages' && segments[1] === 'u') {
    const code = segments[2].toUpperCase()
    if (!PUBLIC_CODE_PATTERN.test(code)) return null
    return { kind: 'private', key: `messages/u/${code}`, path: `/messages/u/${code}`, title: 'بدء محادثة' }
  }
  ...
  if (segments.length === 2 && segments[0] === 'messages') {
    const id = segments[1].toLowerCase()
    if (!isUuid(id)) return null
    return { kind: 'private', key, path: `/messages/${id}`, title: 'الرسائل' }
  }
