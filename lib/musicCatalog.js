...
export async function searchAppleTracks(query, limit = MUSIC_CATALOG_RESULT_LIMIT, signal) {
  ...
  // Reuse server-fetched canonical metadata when the user picks a result.
  // This avoids immediately spending a second Apple request on the same song.
  for (const track of tracks) cacheSet(`lookup:${track.externalId}`, track)
  cacheSet(key, tracks)
  return tracks
}
