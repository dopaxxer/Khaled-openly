...
final class OpenlyTests:XCTestCase {
 ...
 @MainActor func testKeychainSaveAndDelete(){Keychain.save("test-only-session");XCTAssertEqual(Keychain.read(),"test-only-session");Keychain.save(nil);XCTAssertNil(Keychain.read())}
}
